...
/****************************Complete the code!!************************************/
	...
	GrContextFontSet(&sContext,&g_sFontCmss18i);			//设置字符字体
	GrStringDraw(&sContext, "Thumbwheel Voltage", -1, 15,LCD_THUMBWHEEL_LINE, 1);				//以(15,LCD_THUMBWHEEL_LINE）为左上角坐标显示字符“Thumbwheel Voltage”
	GrStringDraw(&sContext, "mV", -1, 280,LCD_THUMBWHEEL_LINE, 1);				//以(280,LCD_THUMBWHEEL_LINE）为左上角坐标显示字符“mV”
	GrStringDraw(&sContext, "Adjustment Scale:", -1, 15,LCD_SETBIT_LINE, 1);				//以(15,LCD_SETBIT_LINE）为左上角坐标显示字符“Adjustment Scale:”
/**********************************************************************************/
...
/****************************Complete the code!!************************************/
			...
			GrStringDrawCentered(&sContext, "  Pause  ", -1, GrContextDpyWidthGet(&sContext)/2,
						 		 130, 1);		//在屏幕正中显示字符“Pause”，并遮盖掉字符“Running”
/**********************************************************************************/
...
/****************************Complete the code!!************************************/
			...
			GrContextForegroundSet(&sContext, ClrWhite);	//设置字符颜色
			GrContextFontSet(&sContext,&g_sFontCmss18i);			//设置字符字体
			GrStringDraw(&sContext, tMessage, -1, 220,LCD_THUMBWHEEL_LINE, 1);				//以(220,LCD_THUMBWHEEL_LINE）为左上角坐标显示实际指轮电压（已被保存在tMessage中）
/**********************************************************************************/
