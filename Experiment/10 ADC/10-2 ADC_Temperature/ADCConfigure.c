...
{
/****************************Complete the code!!************************************/
	SysCtlPeripheralEnable(SYSCTL_PERIPH_ADC0);			//SysCtl使能ADC0模块 
	SysCtlADCSpeedSet(SYSCTL_ADCSPEED_500KSPS);			//设置ADC采样速率为500ksps 
 	HWREG(ADC0_BASE + ADC_O_CTL)=0x01;					//使用外置基准电压
	ADCHardwareOversampleConfigure(ADC0_BASE,64);		//使用ADC过采样功能
	...
	ADCSequenceStepConfigure(ADC0_BASE, 0, 0,
        			  		 ...
        			  		 ADC_CTL_END | 
        			  		 ADC_CTL_IE); 
	ADCIntEnable(ADC0_BASE, 0);        					//ADC模块使能ADC序列0中断 
	IntEnable(INT_ADC0);         						//NVIC模块使能ADC序列0中断 


	ADCSequenceEnable(ADC0_BASE,1);						//使能采样序列1
	...
	ADCSequenceStepConfigure(ADC0_BASE,1,0,ADC_CTL_TS |
										   ADC_CTL_END |
										   ADC_CTL_IE);
	ADCIntEnable(ADC0_BASE, 1);							//ADC模块使能ADC序列1中断
	IntEnable(INT_ADC1);								//NVIC模块使能ADC序列1中断
/************************************************************************************/
