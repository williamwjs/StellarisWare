...
/***********************Complete the Code*************************************/
   SET_LCD_DATA(usData >> 8);		//将待写入数据的高8位送到总线（通过移位获取高位）
	...
	HWREG(LCD_WR_BASE + GPIO_O_DATA + (LCD_WR_PIN << 2   )) =0 ;//拉低写使能信号WR
	HWREG(LCD_WR_BASE + GPIO_O_DATA + (LCD_WR_PIN << 2   )) =0;//为满足写入操作的时序要求，需要将WR重复拉低两次以延时
	...
	HWREG(LCD_WR_BASE + GPIO_O_DATA + (LCD_WR_PIN << 2   )) =0 ;//拉低写使能信号WR
	HWREG(LCD_WR_BASE + GPIO_O_DATA + (LCD_WR_PIN << 2   )) =0 ;//为满足写入操作的时序要求，需要将WR重复拉低两次
...
/***********************Complete the Code*************************************/
    ...
    HWREG(LCD_DC_BASE + GPIO_O_DATA + (LCD_DC_PIN << 2   )) =0 ;//拉低DC

	HWREG(LCD_WR_BASE + GPIO_O_DATA + (LCD_WR_PIN << 2   )) =0 ;//拉低WR
	HWREG(LCD_WR_BASE + GPIO_O_DATA + (LCD_WR_PIN << 2   )) =0 ;//重复上一步
